...
Заголовки пакетов управления MQTT хранятся в максимально коротком формате. Каждый пакет управления протокола состоит следующих частей: фиксированного заголовка, переменного заголовка и полезной нагрузки. Каждый пакет управления имеет 2-5 байтов фиксированного заголовка, однако, не каждый пакет имеет переменный заголовок и полезную нагрузку. 

![Сравнение протоколов MQTT и HTTPS](assets/mqtt_table.png)

Небольшие накладные расходы, связанные с хорошо продуманным форматом заголовков, позволяют эффективно применять этот протокол для решений Интернета вещей. В таблице показаны результаты экспериментального сравннеия протоколов MQTT и HTTPS. В связи с этим протокол MQTT будет использован в данном проекте для транспортировки пакетов данных и команд между устроуствами Интернета вещей и аналитическими средствами облачной платформы IBM Bliemix. 
